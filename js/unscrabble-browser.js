...
class Dictionary {
    ...
    static getDefinition = (word) => {
        return new Promise(resolve => {
            fetch(`${Dictionary.#apiUrl}${word}?key=${Dictionary.#apiKey}`)
            .then(response => response.json())
            .then(data => resolve(
                { word, def: data[0].shortdef }
                ));
            });
     }
}
...
const main = async (input) => {
    await Dictionary.createList('../dictionary.txt');
...
const getTiles = (letters) => {
    ...
    for (let char of letters) {
        ...
        tile.className = 'tile';
        tile.style = (
            `height: 4.5rem; width: 3rem; background-image: url('../images/scrabble-pngs/${
...
const parseData = form => {
    ...
    if (minMax[1].value > length) minMax[1].value = length;
